...
export const mainStateReducer = (draft: MainState, action: MainStateAction): MainState => {
  switch (action.type) {
    ...
    case "SHOW_RESPONDING_COMPONENT": {
      draft.showRespondingComponent = action.payload;
      return draft;
    }

    case "SET_RESPONDING_DATA": {
      draft.respondingData = action.payload;
      return draft;
    }
